locals {
  ...
  backendpool_id2 = {
    for p in azurerm_lb_backend_address_pool.tfmodule:
      element(split("/",p.loadbalancer_id), length(split("/",p.loadbalancer_id))-1) => p.id
        # "${p.loadbalancer_id}/backendAddressPools/${p.name}" => p.id
  }  
  
  #backend_address_pool_id = azurerm_lb_backend_address_pool.tfmodule.id

  # backend_ip_configuration_ids = azurerm_lb_backend_address_pool.beap.backend_ip_configurations.*.id
}
