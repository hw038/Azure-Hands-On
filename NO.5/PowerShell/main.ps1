...
$StrOnVGWPIP = New-AzPublicIpAddress -Name test -ResourceGroupName $OnRgName -Location $OnLocation -AllocationMethod Dynamic -Sku Basic
...
#On NIC
$OnStrVM01IP = New-AzNetworkInterfaceIpConfig -Name $OnVM01Nic -PrivateIpAddressVersion IPv4 -PrivateIpAddress $OnVM01IP -SubnetId $OnStrVnet01.Subnets[0].Id
...
while($IPChk)
{
	if (($ipVGWIP -ne "Not Assigned") -and ($OnipVGWIP -ne "Not Assigned"))
	{
		$IPChk=0
		break
	}
...
New-AzVirtualNetworkGatewayConnection -Name 'OnPrem-to-Azure' -ResourceGroupName $OnRgName -Location $OnLocation -VirtualNetworkGateway1 $OnstrVGW -LocalNetworkGateway2 $Onlocal -ConnectionType IPsec -RoutingWeight 10 -SharedKey 'xptmxm123'
